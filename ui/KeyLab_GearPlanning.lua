...
local SECTIONS = {
    ...
    },
    {
        ...
        body = Join({
            ...
            Tip(),
            "For the best Stat Goal Matcher results, finish and equip your planned 4-piece Tier Set first.\n\nCurrent native Tier pieces are included in the Master Item Database, and the Gear Dashboard recognizes them automatically by item ID.\n\nYou only need four of the five core Tier slots. The matching Back, Wrist, Waist, and Feet pieces are native Tier off-pieces and do not count toward the set bonus.",
        }),
    },
    {
    ...
    },
    {
        ...
        body = Join({
            ...
            Tip(),
            "Equip the trinkets you want to keep before running the Stat Goal Matcher. Trinket secondary stats are excluded from the goal projection.\n\nIf a trinket slot is empty, KeyLab may show separate advisory Stat Support suggestions for dungeon or raid trinkets with secondary stats.\n\nKeyLab does not judge trinket effects. Stat Support is not a Best in Slot claim.",
        }),
    },
    {
        ...
        body = Join({
            ...
            List({
                ...
                "Open Gear Targets and click Stat Goal Matcher. All matcher settings are now inside this setup pop-up.",
                "Choose Master Item Database or Equipped + Bags as the Item Source.",
                "If using the database, choose Dungeon, Raid, or Dungeon and Raid items.",
                "Use Refresh Current Stats in the pop-up after changing equipment if the current-stat comparisons need updating.",
                "Optionally choose your spec's primary stat in Primary first. It becomes the #1 requirement; None keeps the secondary-only class-wide search.",
                "If Weapon Setup appears, choose Two-Handed or Dual Wield. A required setup is shown automatically for specs whose core abilities depend on it.",
                ...
                "Use ^ / v to order your secondary-stat priorities. Goals and priorities are saved for the spec you are planning.",
                "Choose Balanced for the smallest total gap, or Favor Priority to give more weight to your higher-priority stats.",
                "When the short countdown ends, click Start Matcher while out of combat and review the suggested set. Results reopens the last report.",
                "Mark any item you want as a Target or Alternative.",
            }),
            "KeyLab locks your equipped items, fills eligible empty slots, and projects the finished set. It searches independently of the visible Gear Targets list: browsing filters and saved Targets do not define its candidate pool. Database items are normalized for comparison; known upgrade tracks on owned items are projected to their highest rank.",
            Heading("Class-Wide Gear and Loot Spec"),
            "The database search includes recorded loot from every spec of your class, not just your active spec. Class and weapon-slot restrictions still apply. An item from another spec is not automatically a recommendation for your role; check its stats and effects.\n\nLoot Spec beneath an item lists its recorded drop specs. The raid/M+ Gear Targets pop-ups also show Loot Spec Set and mark mismatches in orange with 'change needed.' Use Preparation Panel to change loot spec before seeking an item. Changing loot spec does not switch your talent build, and raid loot rules may differ.",
            Heading("Secondary Stats and Trinkets"),
            "Primary first offers None or the primary stat for your spec: Agility, Intellect, or Strength. Choosing it requires new armor and weapon candidates to provide that stat for your spec before the matcher compares secondary goals. It has no percentage field and does not maximize the total amount of primary stat.\n\nFor example, Feral's Agility requirement excludes Intellect-only weapons, but keeps shared armor that provides Agility to Feral and eligible Agility gear shared with Guardian. Item rules still apply; equipping an item does not change its fixed primary stat. Candidates with unconfirmed primary stats are excluded while the requirement is enabled.\n\nRings and necklaces remain eligible without a primary stat. Equipped items stay locked. Trinket handling is unchanged: trinket secondary stats are excluded from the goal projection, and empty trinket slots may receive separate advisory Stat Support suggestions.\n\nPercentage goals remain Crit, Haste, Mastery, and Versatility. Stamina is not a matcher priority. The main page's primary-stat checkboxes remain browsing filters only.",
            Heading("Weapon Setup Rules"),
            "Retribution Paladin, Arms Warrior, Unholy Death Knight, and Survival Hunter require a two-handed setup. Enhancement Shaman requires two compatible one-handed weapons. Frost Death Knight, Brewmaster Monk, and Windwalker Monk must choose Two-Handed or Dual Wield in matcher setup; that choice is remembered for the character and spec.\n\nKeyLab evaluates only complete legal configurations. A two-handed weapon closes the off-hand slot. Dual Wield requires both a main-hand and off-hand weapon; a shield or caster off-hand does not count. Equipped weapons remain locked, so KeyLab asks you to unequip a weapon that conflicts with the chosen setup. Other specs keep their existing weapon rules.",
            Heading("Preparation Panel Quick Access"),
            "Minimize the main addon to open Preparation Panel, available solo or grouped. Check or switch saved talent builds, set loot spec, open crafting or gear shopping lists, and choose a full addon page from Go To. Grouped players also see readiness and present Class/Spec capabilities.\n\nUse the minus button to shrink the panel to the PREP PANEL handle; click the handle to reopen it while out of combat. Both can be moved. They hide in combat and the handle returns afterward. Talent switching is unavailable during an active M+ key.",
    ...
    },
    {
        ...
        body = Join({
            ...
            List({
                ...
                "Which core Tier slots are currently equipped",
                "Automatic Crafted, native Tier off-piece, and Other Item labels",
            ...
            }) .. "\n\nYou only need four of the five core Tier slots. KeyLab reads the equipped item IDs and updates 1-piece, 2-piece, 3-piece, and 4-piece progress automatically.",
            Tip(),
