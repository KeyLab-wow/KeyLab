...
local function ApplyStatus(item, status, slotInstance)
    ...
    elseif status == "target" then
        ok, reason = targets.SetTargetForSlot(TargetSpecID(), item.itemID, slotInstance, item.sourceID, true)
    elseif status == "alternative" then
        ok, reason = targets.SetAlternativeForSlot(TargetSpecID(), item.itemID, slotInstance, item.sourceID)
    end
...
local function EligibleStatusSlots(item, status)
    ...
    if status ~= "alternative" then return slots end
    local out = {}
...
local function OpenSlotChoice(anchor, item, status)
    local slots = EligibleStatusSlots(item, status)
    if #slots == 0 then
        PrintAssignmentError(status == "alternative" and "target_required" or "invalid_slot")
        return
    end
...
function GearTargets:RefreshMatcherCard()
    ...
    local total = KeyLab.StatGoalsDB.GetTotal(TargetSpecID())
    local valid = math.abs(total - 100) <= 0.001
    self.goalTotal:SetText(string.format("Total: %.1f%%", total))
    self.goalTotal:SetTextColor(unpack(valid and CFG.colors.green or CFG.colors.warning))

    local matcher = KeyLab.StatGoalMatcher
    local currentShares, currentTotal = {}, 0
    if matcher and matcher.GetCurrentShares then
        local shares, _, totalRating = matcher.GetCurrentShares()
        currentShares = type(shares) == "table" and shares or {}
        currentTotal = tonumber(totalRating) or 0
    end
    ...
    for index, statKey in ipairs(displayOrder) do
        ...
        if row then
            ...
            local current = currentTotal and currentTotal > 0 and tonumber(currentShares[statKey]) or nil
    ...
    if matcher and matcher.IsRunning() then
        ...
        self.matcherState:SetText(self.matcherProgressText or "Calculating...")
    ...
    if result then
        ...
        local message = string.format("Goal Match ready: %d slot(s), %s, %s search, %.2f difference.",
            matched, ItemTypeLabel(result.itemType), tostring(result.mode or ""), tonumber(result.difference) or 0)
        if unmatched > 0 then
            message = string.format("Goal Match ready: %d slot(s) using %s. No distinct candidate for: %s.",
                matched, ItemTypeLabel(result.itemType), table.concat(result.unmatchedOpenSlots, ", "))
        end
    ...
    elseif valid then
        ...
        self.matcherState:SetText("The four goals must total exactly 100% before the matcher can run.")
        self.matcherState:SetTextColor(unpack(CFG.colors.warning))
        self:SetMatcherStatusStyle("warning")
    end
end
...
function GearTargets:CreatePreparationPopup()
    ...
    local body = MakeText(popup,
        ...
        "Choose whether KeyLab should search dungeon items, raid items, or both. The matcher uses non-Tier items from KeyLab's Master Item Database and compares secondary-stat percentage gaps only. It does not judge item level, bonuses, effects, or Best in Slot.",
        "GameFontHighlightSmall", nil, CFG.colors.text)
    body:SetPoint("TOPLEFT", popup, "TOPLEFT", 24, -58)
    body:SetSize(522, 145)
    body:SetWordWrap(true)

    popup.scopeDropdown = MakeDropdown(popup, 210, 24, -216, "Items to Include", function(_, level)
    ...
    scopeHelp:SetPoint("TOPLEFT", popup, "TOPLEFT", 260, -220)
    scopeHelp:SetSize(282, 42)
    scopeHelp:SetWordWrap(true)

    popup.countdown = MakeText(popup, "", "GameFontHighlightSmall", nil, CFG.colors.blue, "CENTER")
    popup.countdown:SetPoint("TOP", popup, "TOP", 0, -286)
...
function GearTargets:OpenPreparationPopup()
    ...
    SetDropdownText(popup.scopeDropdown, ItemTypeLabel(self.matcherItemType))
...
function GearTargets:StartMatcher()
    ...
    self.matcherProgressText = "Calculating..."
    self:RefreshMatcherCard()
    local started, message = matcher.Start({
        ...
        GearTargets.matcherProgressText = total and total > 0
            and string.format("Calculating... %s %d / %d", mode, completed, total)
            or ("Calculating... " .. mode)
        GearTargets:RefreshMatcherCard()
    end, function(payload)
        GearTargets.matcherProgressText = payload and not payload.ok and payload.message or nil
...
function GearTargets:Create(parent)
    ...
    matcherTitle:SetSize(360, 18)
    local matcherNote = MakeText(matcherPanel, "Set your combined secondary-stat percentages. Use the arrows to change their priority order.", "GameFontDisableSmall", nil, CFG.colors.muted)
    ...
    self.goalTotal = MakeText(matcherPanel, "Total: 0%", "GameFontHighlightSmall", nil, CFG.colors.warning)
    self.goalTotal:SetPoint("TOPLEFT", matcherPanel, "TOPLEFT", 14, -116)
    self.goalTotal:SetSize(130, 18)
    local goalHint = MakeText(matcherPanel, "Must equal 100%", "GameFontDisableSmall", nil, CFG.colors.muted)
