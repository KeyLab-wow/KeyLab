...
local function getTooltipText(item)
    if not item or not item.link or not C_TooltipInfo or not C_TooltipInfo.GetHyperlink then
        return ""
    end

    local key = item.itemID or item.link
    if tooltipTextCache[key] ~= nil then return tooltipTextCache[key] end

    local lines = {}
    local ok, tooltip = pcall(C_TooltipInfo.GetHyperlink, item.link)
    if ok and tooltip and type(tooltip.lines) == "table" then
        for _, line in ipairs(tooltip.lines or {}) do
            if line.leftText then table.insert(lines, tostring(line.leftText)) end
            if line.rightText then table.insert(lines, tostring(line.rightText)) end
        end
    end

    tooltipTextCache[key] = table.concat(lines, " ")
    return tooltipTextCache[key]
end
